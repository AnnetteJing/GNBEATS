...
parser.add_argument('--device', type=str) # "cuda:0" if torch.cuda.is_available() else "cpu"
...
parser.add_argument('--node_mod', action=argparse.BooleanOptionalAction, default=True)
# Whether to share parameters across blocks
parser.add_argument('--share_params', action=argparse.BooleanOptionalAction, default=True)
...
# Ordering of blocks
parser.add_argument('--static_block_order', nargs='+', type=int) # [0, 1]
parser.add_argument('--ar_block_order', nargs='+', type=int) # [0, 1, 2, 0, 0, 1, 1, 2, 2]
parser.add_argument('--graph_block_order', nargs='+', type=int) # [0, 1, 2, 0, 0, 1, 1, 2, 2]
...
parser.add_argument('--trend_id', action=argparse.BooleanOptionalAction, default=False)
parser.add_argument('--season_id', action=argparse.BooleanOptionalAction, default=False)
# Whether to use average & individual static basis coefficients
parser.add_argument('--static_theta_avg', action=argparse.BooleanOptionalAction, default=True)
parser.add_argument('--static_theta_individual', action=argparse.BooleanOptionalAction, default=True)
# Which network to use for dynamic basis coefficients
...
if __name__ == '__main__':
    ...
    print("Testing model...")
    model.test(loader, normalizer, return_output=False)
